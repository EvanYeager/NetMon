...
void ICMP::startPings() {
  ...
  std::unique_ptr<FILE, void(*)(FILE*)> pipe(popen("ping 8.8.8.8 -c 100 -i .2 -v 2>/dev/null", "r"),
    [](FILE * f) -> void
    {
  ...
  while (fgets(buffer.data(), static_cast<int>(buffer.size()), pipe.get()) != nullptr) {
    ...
    if (seqStart != std::string::npos && seqEnd != std::string::npos) {
      ...
      int numLost = seq - (lastSeq + 1); // if this seq is more than one greater than previous, it lost one
